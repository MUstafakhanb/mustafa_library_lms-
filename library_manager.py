    ...
    try:
        if os.path.exists('library.json'):
            with open('library.json', 'r') as file:
                st.session_state.library_data = json.load(file)
                return True
            return False
...
def save_library():
    try:
        with open('library.json', 'w') as file:
            json.dump(st.session_state.library_data, file)
        return True
    except Exception as e:
...
if st.session_state.current_view == "add":
    ...
    with st.form(key='add_book_form'):
        ...
        with col1:
            ...
            publication_year = st.number_input("Publicaton year",min_value=1000, max_value=datetime.now().year, step=1, value=2023)

        with col2:
            genre = st.selectbox("Genre",[
                "Friction","Non-friction","Science","Fantasy","Mystery","Romance","Thriller","Horror","Biography","History","Travel","Cooking","Art","Self-help","Other"
            ])
                ...
                if st.session_state.book_removed:
                    ...
                    if hasattr(st.session_state, 'search_results'):
                        if st.session_state.search_results:
                            st.markdown(f"<h1> Found {len(st.session_state.search_results)} results:</h1>",unsafe_allow_html=True)
...
if not st.session_state.library:
            ...
            if stats['authors']:
                ...
                st.markdown("---")
                st.markdown("Copyright @ 2025 Personal Library Management System", unsafe_allow_html=True)
